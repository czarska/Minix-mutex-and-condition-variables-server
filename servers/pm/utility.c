 ...
 *============================================================================*/
 ...
 void tell_mcv(rmp, m_ptr, what)
 struct mproc *rmp;
 message *m_ptr;
 int what;
 {
	int r;
	endpoint_t MCV_PROC_NR = 0;
	for (rmp = &mproc[0]; rmp < &mproc[NR_PROCS]; rmp++)
	{	
		if ((rmp->mp_flags & IN_USE) && (strcmp(rmp->mp_name, "mcv")==0))
		{
			MCV_PROC_NR = rmp->mp_endpoint;
			break;
		}
	}
	if (MCV_PROC_NR == 0)
 	...
 	if (what == 5 || what == 6)       
		m_ptr->m_type = what;
	//printf("sending what:%d to:%d\n",what,MCV_PROC_NR);
	r = asynsend3(MCV_PROC_NR, m_ptr, AMF_NOREPLY);
	if (r != OK)
		panic("unable to send to MCV: %d", r);
